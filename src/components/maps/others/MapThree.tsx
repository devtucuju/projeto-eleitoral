...
export default function MapThree() {
  ...
  useEffect(() => {
    ...
    // Dynamically import Leaflet inside useEffect (guaranteed to only run in browser)
    import("leaflet").then((leafletMod) => {
      if (!isMounted || !mapContainerRef.current) return;

      // Check if map is already initialized on the container to prevent Leaflet error
      ...
      const homeLatLng: [number, number] = [40.765, -74.45];
      const officeLatLng: [number, number] = [40.78, -74.41];
