import { NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";
import { requireAdmin } from "@/lib/admin-auth";

// PATCH /api/admin/candidatos/[id]
export async function PATCH(
  req: Request,
  { params }: { params: Promise<{ id: string }> }
) {
  const auth = await requireAdmin();
  if (auth instanceof NextResponse) return auth;
  try {
    const { id } = await params;
    const body = await req.json();
    const candidato = await prisma.candidato.update({
      where: { id },
      data: {
        ...(body.nome !== undefined && { nome: String(body.nome) }),
        ...(body.apelido !== undefined && { apelido: body.apelido ? String(body.apelido) : null }),
        ...(body.numero !== undefined && { numero: Number(body.numero) }),
        ...(body.cargo !== undefined && { cargo: String(body.cargo) }),
        ...(body.foto !== undefined && { foto: body.foto ? String(body.foto) : null }),
        ...(body.ativo !== undefined && { ativo: Boolean(body.ativo) }),
      },
    });
    return NextResponse.json({ ok: true, candidato });
  } catch (err) {
    return NextResponse.json({ error: "Erro interno" }, { status: 500 });
  }
}

// DELETE /api/admin/candidatos/[id]
export async function DELETE(
  _req: Request,
  { params }: { params: Promise<{ id: string }> }
) {
  const auth = await requireAdmin();
  if (auth instanceof NextResponse) return auth;
  try {
    const { id } = await params;
    await prisma.candidato.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (err: any) {
    if (err.code === "P2003") {
      return NextResponse.json({ error: "Candidato tem células ou missões vinculadas" }, { status: 409 });
    }
    return NextResponse.json({ error: "Erro interno" }, { status: 500 });
  }
}
